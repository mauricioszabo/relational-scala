...
class AttributeTest extends WordSpec with AdapterSetup {
  ...
  "Attribute" should {
    ...
    "represent a literal" in {
      pseudoSQL(attributes.Attribute.wrap(10)) should be === "10"
      pseudoSQL(attributes.Attribute.wrap("foo")) should be === "'foo'"
      pseudoSQL(attributes.Attribute.wrap(name)) should be === "examples.name"
    }
  }
  ...
  def pseudoSQL(a: attributes.AttributeLike) = a.partial.toPseudoSQL
}
